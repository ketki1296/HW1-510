...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		await listBranches(userId,"HW0-510");
		await createRepo(userId,"trial");
		await createIssue(userId, "Coding_Prompt", "Found a bug", "Having a problem with this");
		await enableWikiSupport(userId,"Coding_prompt");
...
function getDefaultOptions12(endpoint, method, repodata)
{
        var options = {
                url: urlRoot + endpoint,
                method: method,
...
{	
	var repodata = {"name" : repo};
	let options = getDefaultOptions12("/user/repos", "POST", repodata);
	// Send a http request to url and specify a callback that will be called upon its return.
	return new Promise(function(resolve, reject)
	{
		request(options, function (error, response, body) {
...
{	
	var repodata = {"title":issueName,"body":issueBody};
	let options = getDefaultOptions12("/repos/"+owner+"/"+repo+"/issues", "POST", repodata);
...
{	
	var repodata = {"has_wiki": "true"};
	let options = getDefaultOptions12("/repos/"+owner+"/"+repo, "PATCH", repodata);
